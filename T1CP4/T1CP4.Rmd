...
## Distribuição dos gastos da Piauí por mês, de Janeiro a Maio
```{r}
...
```{r}
empresas.beneficiadas = dados %>%
  select(txNomeParlamentar, numSubCota, txtFornecedor, vlrLiquido) %>%
  group_by(txNomeParlamentar, txtFornecedor) %>%
  filter(numSubCota == 1, txtFornecedor == ".CASA DOS SÍNDICOS SE LTDA") %>%
  summarise(valor = sum(vlrLiquido), qtdDeVezes = n())


head(empresas.beneficiadas)
